 ...
 !!!!  Don't change anything above this line.  Start modifying below.
	...
	T_tau = 2.d0 * mu * Ee0 + kappa * tr_Ee * I_1
	 ...
	 state_new(km,20)=1.d0
   	   ...
   	   Fp_tau = matmul(matmul(
     +  		matmul(eigvecs, reshape((/ 
     +		  dexp(stretches(1,1)*dt), z, z,
     +	  z, dexp(stretches(2,2)*dt), z,
     +  		z, z, dexp(stretches(3,3)*dt) /), (/3, 3/))),
     +	  transpose(eigvecs)), Fp_t)
